import { Gamma } from "./gamma";

export class DES {
	protected key: string = '';
	protected subkeys: string[] = [];
	protected avalanche: string[] = [];
	protected avalanchePcbc: string[][] = [];

	constructor(key: string) {
		this.key = key;
		this.prepareSubkeys();
	}

	protected prepareSubkeys(): void {
		const KEY_PERMUTATION_TABLE = [
			'57', '49', '41', '33', '25', '17', '09',
			'1', '58', '50', '42', '34', '26', '18',
			'10', '2', '59', '51', '43', '35', '27',
			'19', '11', '3', '60', '52', '44', '36',
			'63', '55', '47', '39', '31', '23', '15',
			'7', '62', '54', '46', '38', '30', '22',
			'14', '6', '61', '53', '45', '37', '29',
			'21', '13', '5', '28', '20', '12', '4'
		];
		const Gk = KEY_PERMUTATION_TABLE
			.map(permute => this.key.slice(+permute - 1, +permute))
			.join('');

		const C = [Gk.slice(0, 56 / 2)];
		const D = [Gk.slice(-56 / 2)];

		{
			const SHIFTS_TABLE = [
				false, false, true, true, true, true, true, true,
				false, true, true, true, true, true, true, false
			];
			const cycleShift = (str: string, shift: number): string =>
				str.slice(shift) + str.slice(0, shift);
	
			for (let i = 1; i <= 16; i++) {
				C[i] = cycleShift(C[i - 1], 1 + +SHIFTS_TABLE[i - 1]);
				D[i] = cycleShift(D[i - 1], 1 + +SHIFTS_TABLE[i - 1]);
			}
		}

		{
			const POST_PERMUTATION = [
				'14', '17', '11', '24', '1', '5', '3', '28',
				'15', '6', '21', '10', '23', '19', '12', '4',
				'26', '8', '16', '7', '27', '20', '13', '2',
				'41', '52', '31', '37', '47', '55', '30', '40',
				'51', '45', '33', '48', '44', '49', '39', '56',
				'34', '53', '46', '42', '50', '36', '29', '32'
			];
			for (let i = 0; i < 16 + 1; i++) {
				const CD = (C[i] + D[i]).split('');
				this.subkeys[i] = POST_PERMUTATION
					.map(permute => CD[+permute - 1])
					.join('');
			}
		}
	}

	protected getSubkey(i: number): string {
		return this.subkeys[i];
	}

	protected generationFunction(rBits: string, subkey: string): string {
		const extend = (bitsStr: string): string => {
			const bitsArr = bitsStr.split('');
			const EXTENSION_TABLE = [
				'32', '1', '2', '3', '4', '5',
				'4', '5', '6', '7', '8', '9',
				'8', '9', '10', '11', '12', '13',
				'12', '13', '14', '15', '16', '17',
				'16', '17', '18', '19', '20', '21',
				'20', '21', '22', '23', '24', '25',
				'24', '25', '26', '27', '28', '29',
				'28', '29', '30', '31', '32', '1'
			];

			return EXTENSION_TABLE
				.map(extend => bitsArr[+extend - 1])
				.join('');
		}

		const extendedRBits = extend(rBits);
		const B: string[] = Gamma.apply(subkey, extendedRBits)
			.match(/[01]{1,6}/g) || [];

		const S_8_TO_6_TABLES = [
			[
				['14', '4', '13', '1', '2', '15', '11', '8', '3', '10', '6', '12', '5', '9', '0', '7'],
				['0', '15', '7', '4', '14', '2', '13', '1', '10', '6', '12', '11', '9', '5', '3', '8'],
				['4', '1', '14', '8', '13', '6', '2', '11', '15', '12', '9', '7', '3', '10', '5', '0'],
				['15', '12', '8', '2', '4', '9', '1', '7', '5', '11', '3', '14', '10', '0', '6', '13']
			],
			[
				['15', '1', '8', '14', '6', '11', '3', '4', '9', '7', '2', '13', '12', '0', '5', '10'],
				['3', '13', '4', '7', '15', '2', '8', '14', '12', '0', '1', '10', '6', '9', '11', '5'],
				['0', '14', '7', '11', '10', '4', '13', '1', '5', '8', '12', '6', '9', '3', '2', '15'],
				['13', '8', '10', '1', '3', '15', '4', '2', '11', '6', '7', '12', '0', '5', '14', '9']
			],
			[
				['10', '0', '9', '14', '6', '3', '15', '5', '1', '13', '12', '7', '11', '4', '2', '8'],
				['13', '7', '0', '9', '3', '4', '6', '10', '2', '8', '5', '14', '12', '11', '15', '1'],
				['13', '6', '4', '9', '8', '15', '3', '0', '11', '1', '2', '12', '5', '10', '14', '7'],
				['1', '10', '13', '0', '6', '9', '8', '7', '4', '15', '14', '3', '11', '5', '2', '12']
			],
			[
				['7', '13', '14', '3', '0', '6', '9', '10', '1', '2', '8', '5', '11', '12', '4', '15'],
				['13', '8', '11', '5', '6', '15', '0', '3', '4', '7', '2', '12', '1', '10', '14', '9'],
				['10', '6', '9', '0', '12', '11', '7', '13', '15', '1', '3', '14', '5', '2', '8', '4'],
				['3', '15', '0', '6', '10', '1', '13', '8', '9', '4', '5', '11', '12', '7', '2', '14']
			],
			[
				['2', '12', '4', '1', '7', '10', '11', '6', '8', '5', '3', '15', '13', '0', '14', '9'],
				['14', '11', '2', '12', '4', '7', '13', '1', '5', '0', '15', '10', '3', '9', '8', '6'],
				['4', '2', '1', '11', '10', '13', '7', '8', '15', '9', '12', '5', '6', '3', '0', '14'],
				['11', '8', '12', '7', '1', '14', '2', '13', '6', '15', '0', '9', '10', '4', '5', '3']
			],
			[
				['12', '1', '10', '15', '9', '2', '6', '8', '0', '13', '3', '4', '14', '7', '5', '11'],
				['10', '15', '4', '2', '7', '12', '9', '5', '6', '1', '13', '14', '0', '11', '3', '8'],
				['9', '14', '15', '5', '2', '8', '12', '3', '7', '0', '4', '10', '1', '13', '11', '6'],
				['4', '3', '2', '12', '9', '5', '15', '10', '11', '14', '1', '7', '6', '0', '8', '13']
			],
			[
				['4', '11', '2', '14', '15', '0', '8', '13', '3', '12', '9', '7', '5', '10', '6', '1'],
				['13', '0', '11', '7', '4', '9', '1', '10', '14', '3', '5', '12', '2', '15', '8', '6'],
				['1', '4', '11', '13', '12', '3', '7', '14', '10', '15', '6', '8', '0', '5', '9', '2'],
				['6', '11', '13', '8', '1', '4', '10', '7', '9', '5', '0', '15', '14', '2', '3', '12']
			],
			[
				['13', '2', '8', '4', '6', '15', '11', '1', '10', '9', '3', '14', '5', '0', '12', '7'],
				['1', '15', '13', '8', '10', '3', '7', '4', '12', '5', '6', '11', '0', '14', '9', '2'],
				['7', '11', '4', '1', '9', '12', '14', '2', '0', '6', '10', '13', '15', '3', '5', '8'],
				['2', '1', '14', '7', '4', '10', '8', '13', '15', '12', '9', '0', '3', '5', '6', '11']
			]
		];

		const S: string[] = B
			.map((bits6: string, sIndex: number): string => {
				const row = parseInt(bits6.slice(0, 1) + bits6.slice(-1), 2);
				const column = parseInt(bits6.slice(1,5), 2);

				const reducedRes = (+S_8_TO_6_TABLES[sIndex][row][column]).toString(2);
				return '0'.repeat(4 - reducedRes.length) + reducedRes;
			})
			.join('')
			.split('');

		const FINAL_PERMUTATION_TABLE = [
			'16', '7', '20', '21', '29', '12', '28', '17',
			'1', '15', '23', '26', '5', '18', '31', '10',
			'2', '8', '24', '14', '32', '27', '3', '9',
			'19', '13', '30', '6', '22', '11', '4', '25'
		];
		return FINAL_PERMUTATION_TABLE
			.map(permutation => S[+permutation - 1])
			.join('');
	}

	protected permute(bits64: string, reverse = false): string {
		const bitsArray: string[] = bits64.split('');
		const BASE_PERMUTATION_TABLES = [
			[	// Direct
				'58', '50', '42', '34', '26', '18', '10', '2',
				'60', '52', '44', '36', '28', '20', '12', '4',
				'62', '54', '46', '38', '30', '22', '14', '6',
				'64', '56', '48', '40', '32', '24', '16', '8',
				'57', '49', '41', '33', '25', '17', '9', '1',
				'59', '51', '43', '35', '27', '19', '11', '3',
				'61', '53', '45', '37', '29', '21', '13', '5',
				'63', '55', '47', '39', '31', '23', '15', '7'
			], [// Reverse
				'40', '8', '48', '16', '56', '24', '64', '32',
				'39', '7', '47', '15', '55', '23', '63', '31',
				'38', '6', '46', '14', '54', '22', '62', '30',
				'37', '5', '45', '13', '53', '21', '61', '29',
				'36', '4', '44', '12', '52', '20', '60', '28',
				'35', '3', '43', '11', '51', '19', '59', '27',
				'34', '2', '42', '10', '50', '18', '58', '26',
				'33', '1', '41', '9', '49', '17', '57', '25',
			]
		];

		return BASE_PERMUTATION_TABLES[reverse ? 1 : 0]
			.map(permute => bitsArray[+permute - 1])
			.join('');
	}

	protected iterateBlocks = (blocks: string[], i: number): string[] => {
		const left32: string = blocks[0];
		const right32: string = blocks[1];
		const Vi: string = this.getSubkey(i);

		const newRight32: string = Gamma.apply(
			this.generationFunction(right32, Vi),
			left32
		);
		const newLeft32: string = right32;

		return [
			newLeft32,
			newRight32
		];
	}

	protected encryptBlock(bits64: string, direct: boolean): string {
		this.avalanche[0] = this.avalanche[0] ? this.avalanche[0] + bits64 : bits64;
		let blocks: string[] = this.permute(bits64).match(/.{1,32}/g) || [];

		for (let i = direct ? 1 : 16;
			direct ? i <= 16 : i >= 1;
			i += direct ? 1 : -1) {
			blocks = this.iterateBlocks(blocks, i);
			this.avalanche[i] = this.avalanche[i] ?
				this.avalanche[i] + blocks.join('') :
				blocks.join('');
		}

		const resCipher = this.permute(blocks.reverse().join(''), true);

		this.avalanche[17] = this.avalanche[17] ?
			this.avalanche[17] + resCipher :
			resCipher;
		return resCipher;
	}

	protected E(bits64: string): string {
		return this.encryptBlock(bits64, true);
	}

	protected D(bits64: string): string {
		return this.encryptBlock(bits64, false);
	}

	apply(bits: string, direct = true): string {
		this.avalanche = [];
		return (('0'.repeat(bits.length % 64 ? 64 - bits.length % 64 : 0) + bits)
			.match(/.{1,64}/g) || [])
			.map((bits64: string): string => this.encryptBlock(bits64, direct))
			.join('');
	}

	protected pcbcEncrypt(bits: string, IVc: string, IVp: string): string {
		let blocks = ('0'.repeat(bits.length % 64 ? 64 - bits.length % 64 : 0) + bits)
			.match(/.{1,64}/g) || [];
		const P = [IVp, ...blocks];
		const C = [IVc];

		for (let i = 1; i < P.length; i++) {
			const toCipher = Gamma.apply(Gamma.apply(P[i], C[i - 1]), P[i - 1]);
			C[i] = this.E(toCipher);
			this.avalanchePcbc.push(this.avalanche);
			this.avalanche = [];
		}

		return C.join('');
	}

	protected pcbcDecrypt(bits: string, IVp: string): string {
		let blocks = ('0'.repeat(bits.length % 64 ? 64 - bits.length % 64 : 0) + bits)
			.match(/.{1,64}/g) || [];
		const P = [...blocks];
		const C = [IVp];

		for (let i = 1; i < P.length; i++) {
			C[i] = Gamma.apply(
				this.D(P[i]),
				Gamma.apply(C[i - 1], P[i - 1])
			);
			this.avalanchePcbc.push(this.avalanche);
			this.avalanche = [];
		}

		return C.slice(1).join('');
	}

	pcbc(bits: string, IVc: string, IVp: string, direct = true): string {
		this.avalanche = [];
		this.avalanchePcbc = [];

		return direct ?
			this.pcbcEncrypt(bits, IVc, IVp) :
			this.pcbcDecrypt(bits, IVp);
	}

	getAvalanche = (): string[] => this.avalanche;
	getAvalanchePcbc = (): string[][] => this.avalanchePcbc;
}
